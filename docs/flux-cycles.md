...
* `logIn`
  0. invoked from `SignInPage` `onSubmit`
  0. `POST /api/session` is called.
  0. `receiveCurrentUser` is set as the callback.
* `logOut`
  0. invoked from `SignInPage` `onClick`
...
* `fetchSearchSuggestions`
  0. invoked from `NavSearchBar` `onChange` when there is text
  0. `GET /api/users` is called with `user_name` param.
...
* `removeSearchSuggestions`
  0. invoked from `NavSearchBar` `onChange` when empty
